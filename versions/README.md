...
| M8.4 | `v0.M8.4` | `b67857e` | 2026-05-30 | — (corpus only) | Strip Hebrew nikud/cantillation (reuses `wikiwatch-M8.3.prg`). Flips M8.1's preserve-nikud. Manifest v8. | 255 + 25 tooling |
| M8.5 | `v0.M8.5` | `f4816d5` | 2026-05-30 | — (corpus only) | Drop "קישורים חיצוניים" + "הערות שוליים" sections; table cells separated by `" | "`; no blank line before first bullet (reuses `wikiwatch-M8.3.prg`). Manifest v9. | 255 + 28 tooling |
| M9 | `v0.M9` | `063a85f` | 2026-05-30 | 186.5 KB | **Manifest-chunking** — lifts the corpus ceiling from ~180 to **1,462 articles (~8.5 MB)**. New `IndexStore` (article index across multiple Storage keys); `Downloader.fetchIndex`; two-phase `InstallView` (9 index parts first, then 1326 body chunks); `KeyboardDelegate` + `wikiwatchApp` read from `IndexStore`. New `pack-index.ps1` corpus tool. `extract.ps1` gains `-SkipExisting`. Server: tiny v10 manifest (0.1 KB, no `articles[]`) + 9 `index/K.json` parts (≤9.6 KB) + 1326 body chunks (≤7.8 KB). Backward-compat proven live (M9 binary + v8 server). | 266 + 30 tooling |
| M9.1 | `v0.M9.1` | `bdadc99` | 2026-05-30 | 187.6 KB | Hotfix two **real-watch crashes** on the M9 forward path (never hit by M9's M8-server sim R2). (1) Install **stack overflow** — `_fireIndexParts` fired all 9 index parts at once; CIQ's ~4-concurrent cap returned `rc=-101` synchronously and the synchronous re-fetch recursed unbounded. Fix: index phase runs through its own `InstallController` (2-in-flight cap, bounded retries, no sync re-fetch). (2) Search **watchdog** — `Search.rank` O(K²) insertion sort over the 1462-article corpus tripped "Code Executed Too Long". Fix: O(n log n) stable merge sort + tier1-fills-TOP_K short-circuit (`rank('ש')` 63 ms). Watch-binary-only (v10 corpus unchanged). | 270 |
| M9.2 | `v0.M9.2` | `c21061c` | 2026-05-30 | 187.6 KB | Proper **70×70 launcher icon** (`resources/drawables/wikiwatch.png`, 32-bit RGBA) replacing the placeholder 24×24 `launcher_icon.svg`. Eliminates the launcher-icon-size build warning (baseline 2 → 1). Resource-only. | 270 |
| M9.3 | `v0.M9.3` | `920536c` | 2026-05-30 | 190.7 KB | Fix the **post-install freeze** on the real watch (M6.5-class GC stall, not OOM — 404 KB still free). The keyboard held all 1462 articles as an `Array<Dictionary>` (~5800 resident objects, 307 KB); replaced with a **compact parallel-array index** (`titles[]`+`pops[]`, position == id == `article:<i>` key) via `IndexStore.loadCompact()` — **77 KB resident, ~4× fewer objects**. Only the ≤50 matched results are materialised per keystroke. Added a **≥3-letter substring gate** (`SUBSTRING_MIN_LEN`) so 1–2-letter broad queries do prefix-only. Cosmetic: install screen shows **"Preparing download…"** during the index/catalog phase instead of the misleading "Loading 0% / 0 of 0". Watch-binary-only (v10 corpus unchanged → re-sideload only). | 283 |
| M9.5 | `v0.M9.5` | `5c7317d` | 2026-05-31 | 196.5 KB | **Per-keystroke search watchdog fix** — the 1000-article corpus crashed on the first letter (CPU-time-per-handler, not RAM/flash; free memory stayed healthy). `Search.rankCompact` now caps each tier at `TOP_K` and **stops the scan once the prefix tier fills** (prefix outranks substring, index is popularity-ordered → no merge-sort of hundreds of entries); `totalMatchesCompact` early-exits at `MATCH_COUNT_CAP=200`. Search is now structurally **N-independent**. Plus: **9 MB install budget** with graceful partial-install stop + "stored X / N" readout (`InstallPlan.shouldStopAtBudget`/`estimateBytes`, `InstallState.installedCount`), **O(1) per-chunk `InstallController`** state machine (was O(received) `indexOf` scans), precomputed `_normTitles` (no per-keystroke `_normalize`), and an absurd-id clamp in `IndexStore.loadCompact`. **Hardware-validated on real Venu 2** (1000 articles, common Hebrew letters, zero crashes). Watch-binary-only (v10 corpus unchanged). | 301 |
| M9.4 | `v0.M9.4` | `f91c5b5` | 2026-05-30 | 194.2 KB | **Survivability harness** after the M9.3 real-watch HARD hang (black screen ~2 h → forced reboot → firmware recovery reset; a CIQ app can't reset a watch — that was firmware self-recovery, but it must never wedge the device again). Adds **`BootGuard`**: a persisted boot breadcrumb set before any heavy index work, cleared when an interactive view shows; **2 consecutive unfinished boots → `SafeModeView`** (zero heavy work; shows free-mem + tap-to-wipe), so a too-large corpus degrades to a recoverable screen instead of crash-looping. Adds an **on-screen free-memory HUD** (`MemHud`) on the install + safe + keyboard screens — the size-sweep instrument the sim can't provide. Tooling: pure **`Get-SelectionCount`** cap + **`build-corpus.ps1 -MaxArticles N -Version V`** to regenerate the server corpus at any size, plus the **size-sweep protocol** (`docs/m9.4-sweep-protocol.md`) to bisect the real hardware ceiling. Watch-binary + tooling. | 292 |
| M9.6 | `v0.M9.6` | `298e423` | 2026-06-01 | 198.2 KB | **Stabilize the 1200-article corpus.** The full 1462 tripped the load-time watchdog (IQ icon + yellow triangle) in the keyboard's index load, so cap at **1200 (v15, 7.13 MB)**. Three real-Venu-2 fixes: (1) **truthful "stored X / N" readout** — `_articlesDisplay` showed `receivedCount * _perChunk`, but `_perChunk` is computed before the article total is known on the chunked path (stays 1) so it showed the chunk count (~314); now real `InstallController.articlesWritten()`. (2) **`IndexCache`** — load the compact index `{titles,pops,normTitles}` ONCE per session, shared across keyboards (`InstallView._freshBegin` clears on wipe); the long-press flow pushed a 2nd keyboard that re-ran the full `loadCompact` + normalize over all N titles on top of the resident reader → heap/watchdog crash at 1200. (3) **`MemGuard`** — refuse opening an article (keyboard suggestion + results row) / long-press in the reader when free heap < 150 KB, with a yellow **"max open articles"** notice (anti uncatchable-OOM from deep view nesting). Improved `build-corpus.ps1` (top-N of the popularity master + chunks-before-index order). Watch-binary + corpus. | 309 |
| M9.7 | `v0.M9.7` | `d64e950` | 2026-06-01 | 200.8 KB | **Keyboard/reader UX polish + close-app.** (1) **`WordSanitize`** — a long-pressed word is stripped to letters (Hebrew U+05D0–05EA / ASCII) + digits + space before it seeds the search box (drops trailing punctuation/quotes/parens/maqaf). (2) the yellow **"max open articles"** notice moved 10 px up on the results list + reader. (3) **keyboard low-memory lockout** — under 150 KB free, the keyboard accepts ONLY the on-screen X (backspace) wedge + the physical back button (typing/expansion/open blocked). (4) **Quit the app from any depth** — long-press the **physical back button** → "Close app?" modal → tap = `System.exit()`, short back = cancel. The Venu 2 firmware turns a *held* back button into a `KEY_MENU` event (found via an on-watch input probe: short back = `KEY_ESC` 5, held = `KEY_MENU` 7), so `onKey()` catches `KEY_MENU` — no manual hold-timing. **Long-press the X wedge clears the whole search box.** New pure `CloseQuery` (modal button geometry). Watch-binary only. | 317 |
| M10.0 | `v0.M10.0` | `5678a6d` | 2026-06-05 | 252.5 KB | **Compression decoder + baked model (inert).** First half of M10 (article-body compression). Ships the byte-level-BPE (vocab-4096) + canonical-Huffman decoder (candidate **E2** from the PC bake-off) and the baked ~36 KB model, but **NOT wired into the read path** — the app behaves exactly as today (zero user-visible risk; the end-to-end format flip + hardware proof is M10.1). New pure `Decompressor` (models/, `Lang`+`StringUtil` only): 24-bit token count → canonical-Huffman bit-walk → token id → append that token's bytes to a `ByteArray` → one `convertEncodedString` at the end (no `String + concat` — the O(N²) OOM trap). New `CompModel` (storage/) loads+parses+caches the baked `resources/jsonData/model.json` once, freeMemory-guarded (R5). The model is generated + **self-verified byte-exact on all 1200 corpus bodies** by `scripts/m10-compress/gen_model.py` (a pure-Python watch-mirror decoder) before the Monkey C port; 11 new golden round-trip `(:test)` incl. the 7680 B largest article. `.prg` +52 KB (the model rides as base64 inside a `jsonData` resource — CIQ has no raw-binary resource type). Sim/`monkeydo`-verified; hardware decode timing is proven when wired in M10.1. | 328 |
| M10.1 | `v0.M10.1` | `50cfa2f` | 2026-06-05 | 261.0 KB | **Wire compression end-to-end, watchdog-safe.** Second half of M10: routes the M10.0 decoder into the article-open path behind a backward-compatible manifest flag, and flips the served corpus to compressed bodies (download **7.13 MB → 2.53 MB, 35.6%**; manifest v15→**v16**, `bodyCodec=bpe-huff-1`/`modelVersion=1`). `Downloader`/`Manifest` parse+persist `bodyCodec`/`modelVersion` (default `plain` → old corpus still works); pure `BodyCodec.readAction` routes `:plain`/`:decompress`/`:unavailable` (decodes only when the corpus modelVersion matches the baked model; mismatch never renders garbage); `ArticleOpener` wires both read paths. **On-hardware watchdog fix:** opening a large article ran the whole model-parse + decode in one event handler → "Code Executed Too Long" on the Venu 2 (the `/t` harness doesn't enforce the watchdog; a `monkeydo` *app* run does — reproducible in-sim). Fixed by a **flat model** (keep model.bin resident + token offsets, 273→137 KB) with the 4096-element table fills **sliced**, plus **incremental decode**, both driven by a new `DecodeView` (~1000 parse items + ~250 decode tokens per Timer tick, 0–100% bar) that then `switchToView`s to the (unchanged lazy-load) reader. New `scripts/compress_corpus.py` (deterministic model, sha-asserted == baked; round-trips all 1200) + **`scripts/watchdog-check.ps1`** (runs the real app in the sim, force-opens the worst-case article, fails on watchdog/crash + asserts lazy-load — run before every test-version). 345 tests (+16). **First CR'd+tagged "test-version"** (per new workflow): sim watchdog-gate green; on-device confirmation happens on this tag. Corpus must be uploaded to wikiwatch.tomhe.app. | 345 |
| M10.2 | `v0.M10.2` | `100d23b` | 2026-06-05 | 266.8 KB | **Streaming decompression — decode the first ~2 screens, show text, stream the rest.** Folds the three M10.2 UX changes into one pass by **merging decode into the reader**. A compressed article no longer decodes its whole body behind a 0–100% bar before any text shows; `wikiwatchView` now holds the decode `state` + parsed `model` and decodes more tokens **on demand** inside its lazy-layout `Timer` loop, so text paints after only the first ~2 screens decode. New pure `Decompressor.decodeTakeLines` drains the raw lines that have become complete in the output buffer, converting only **newline-bounded** byte slices (UTF-8-safe — `0x0A` never appears mid-char; one `convertEncodedString` per call); `splitLines` extracted there as the single definition. New pure `StreamProgress` (`needMoreRawLines` pump gate + `firstPaintShouldContinue` height predicate). First paint becomes a **height target (~2 screens)** capped by a per-tick sub-line budget (was a fixed 2 raw lines); the growing `_rawLines` suppresses the last-line taper until decode is done. **`DecodeView` shrinks to the one-time model-parse gate** (brief `…`, no bar); `ArticleOpener` streams directly when the model is already cached, else routes through the gate. **Watchdog (the hard constraint):** decode + layout share one handler, so per-tick budgets are small (decode 150 tokens/tick, 300 for first paint — 500 trips; layout ≤34 sub-lines) so their **sum** stays under budget. `scripts/watchdog-check.ps1` extended to assert streaming (`tokensDone < tokenCount` at first paint) + ~2-screen first paint. Worst-case id 1143: first paint **32–63 ms**, `contentH=903` (~2.2 screens), `tokensDone=300/1392`, full-layout ~0.6 s, **no watchdog**. 357 tests (+12). CR'd+tagged test-version; on-device confirmation happens on this tag. | 357 |
| M10.3 | `v0.M10.3` | `32ec945` | 2026-06-05 | 268.7 KB | **Eagerly parse the compression model after launch** — so the FIRST compressed article opens without the one-time ~1 s parse gate. New `ModelWarmer` (source/, Timer-driven) parses the model in the background from the keyboard's `onShow` (after `BootGuard.noteReady`, so no boot risk; stopped on `onHide`), sliced across ticks. New `CompModel.parseSlice` drives a **single shared** in-progress parse state used by BOTH the warmer and the `DecodeView` open-gate, so opening an article mid-warm never double-parses / double-allocates the ~137 KB model (the gate resumes the warmer's progress). `CompModel.corpusNeedsModel` gates warming to compressed corpora; `DecodeView` refactored to drive `parseSlice`. New `scripts/warm-check.ps1` (force-opens the keyboard + seeded compressed manifest, asserts `M10.3 warm: model parsed eagerly` + no watchdog); `watchdog-check.ps1` still green (gate fallback unchanged: first paint 47 ms, contentH=903, tokensDone=300/1392). Steady-state memory unchanged — only the 137 KB parse timing moves earlier, behind the 400 KB `MIN_FREE_PARSE` guard. 360 tests (+3). Watch-binary-only (corpus v16 unchanged). On-device confirmation on this tag. | 360 |
| M10.4 | `v0.M10.4` | `edace09` | 2026-06-05 | 270.9 KB | **Recently-read list** — opening an article records it, and an EMPTY search buffer now shows your recently-read articles (most-recent first) so you can jump back without searching (was: empty buffer showed nothing). New pure `Recents.add(list,id,title,cap)` (models/, front-insert + dedup-by-id + cap) + `RecentsStore` (storage/, persists two parallel String arrays under one key, `load`/`record`/`clear`, R4-guarded save, CAP=12). `ArticleOpener.open` gains a `title` param and records the open once it's confirmed openable (both open sites — keyboard suggestion + results row — pass the title; callers already passed the 150 KB MemGuard so "recorded"=="opened"). Surfacing reuses the existing suggestion rows + `▼ N more`→`ResultsView` + tap-to-open (recents are `{:id,:title}`, same shape as ranked results) — no new view. Recents cleared on corpus wipe (`InstallView._freshBegin`). New `scripts/recents-check.ps1` (seeds 2 recents, force-opens the keyboard, asserts `M10.4 recents: n=2` surfaces + no watchdog). 366 tests (+6). Watch-binary-only (corpus v16 unchanged). On-device confirmation on this tag. | 366 |
| M10.5 | `v0.M10.5` | `9d58f5a` | 2026-06-05 | 274.4 KB | **Slice the keyboard index load across ticks** — lifts the corpus cap. The keyboard built the WHOLE search index (deserialize every `index:<K>` part + normalize every title) in ONE event handler; that O(N) work trips the watch watchdog at ~1462 (the reason the corpus was capped at 1200). New `IndexCache.buildStep(maxParts)` advances the build a bounded number of parts per call (normalize interleaved); the keyboard delegate drives it across 50 ms Timer ticks OFF the construction handler. While it loads, the keyboard shows a brief "טוען מאמרים…" hint and **recents (empty buffer) still work** (they don't need the index); the long-press 2nd keyboard reuses the loaded cache; `get()` lazily sync-builds as a fallback. New `IndexStore.rawPart(k)` (raw part, no Symbol-dict conversion). New `scripts/indexload-check.ps1` seeds an 1800-article index (past the old cap) → force-opens the keyboard → proves it loads SLICED (n=1800 ticks=7) + search works + no crash; `warm-check`/`recents-check` still green (no regression). Sim can't trip the watchdog (hardware-only), so the structural slicing is proven in sim+units; final watchdog proof is on-device on this tag (the M9.1 R1 carve-out). **Corpus unchanged (1200, binary-only)** — the read-ranked ~2800-article corpus + faster download is M10.6. 367 tests (+1). | 367 |
| M10.6 | `v0.M10.6` | `1657142` | 2026-06-06 | 275.2 KB | **2,800-article read-ranked corpus + faster install.** Server corpus → **v17**: 2,800 articles (read-ranked by 12-month pageviews, 2.3× the old 1,200) compressed with the **REUSED v1 model** (byte-exact reproduction of baked `model.json`, `modelVersion` stays 1 — model + golden decode tests UNCHANGED, so this binary decodes it), dense-packed to **546 chunks @ 16 KB** + 18 index parts (8.0 MB < 9 MB budget); every body round-trip-verified against the parsed baked decoder. Binary: download concurrency **2 → 4** when memory is ample (`InstallPlan.maxInFlightForMemory` 1/2/4 tiers) + **adaptive `-101` back-off** — queue-full re-queues WITHOUT burning a retry (`InstallController.markRequeue`) and a persistent ceiling ratchets 4→3→2 (`InstallPlan.backoffMaxInFlight`, floor 2) capping the per-tick memory concurrency; + `estimateBytes` 1× fix (compressed bodies are ASCII base64 — 2× falsely tripped the M9.5 budget at ~half the corpus). New `scripts/install-check.ps1` (full sim install over a local HTTPS fixture — the sim requires TLS) empirically found the sim `-402` response cap ≈ **17 KB** (not the hoped ~64 KB), so 16 KB packs with margin and fully installs (546 chunks, no -402/-101/crash); `indexload-check`→2,800 (sliced load, 11 ticks); `watchdog-check` auto-picks the largest shipped blob (id=2251). **Corpus is server-side** — user uploads `docs/server/` to wikiwatch.tomhe.app; the watch sees v17 > v16 → wipe+reinstall. Hardware-only on this tag: the real 16 KB watch cap, watchdog at 2,800, the `-101` trigger, BLE speed. 373 tests (+6). | 373 |
| M10.7 | `v0.M10.7` | `9c56c4a` | 2026-06-06 | *(reuses M10.6)* | **Dense 48 KB chunks — the real watch cap is ~48 KB, not 16 KB.** Corpus-only follow-up (no `.mc` change → reuses `wikiwatch-M10.6.prg`, like M8.1/M8.2). On-device probing (a standalone **CapProbe** app + exact-size payloads) found the Venu 2 `makeWebRequest` response cap is between **48 KB and 64 KB** (48 KB→200, 64 KB→−402, ×3) — far above the simulator's **16 KB** and the old 13.8 KB assumption, so the dense-chunk download win is real on hardware. Re-packs the **same 2,800 bodies** (byte-identical, verified same SHA-256; reused v1 model, `modelVersion` 1) from 546 chunks @ 16 KB → **169 chunks @ 48 KB** → manifest **v18**: *half* the old 1,200-corpus's 254 chunks for 2.3× the content, at concurrency 4. Index parts unchanged (18). 8.0 MB (< 9 MB). The sim CANNOT validate 48 KB (sim cap 16 KB < watch 48 KB) — that gap is why CapProbe exists; bodies/index/machinery/watchdog inherit M10.6's green gates (identical inputs), every body round-trip-verified during packing. Tooling: `install-check.ps1 -SimPackKB` (sim-safe re-pack to keep the machinery gate green when shipped chunks exceed the sim cap), `dense_pack.py --out-dir`, `build_v17.ps1 -Version`. **User uploads `docs/server/` (v18) → watch sees v18 > v16 → wipe+reinstall.** 373 tests (unchanged — corpus-only). | 373 |
| M10.8 | `v0.M10.8` | `0a37893` | 2026-06-06 | 278.3 KB | **Close-app from anywhere + install download telemetry.** (1) M9.7's long-press-back (`KEY_MENU`) → "Close app?" modal was wired only into the keyboard; extend it to the **article reader** (`wikiwatchView`/`Delegate`) and **results list** (`ResultsView`/`Delegate`) so holding back quits from any screen (tap button = `System.exit`, short back/elsewhere = cancel). New shared renderer `CloseQueryUi.draw` (in `source/`, not `models/` — it draws — R6); reuses the pure tested `CloseQuery.buttonHit`; each delegate gates other input while the modal is up. (2) The v18 48 KB install felt slow on-device; added an on-screen **elapsed-seconds + effective-concurrency** HUD (`"<N>s  x<inflight>"`, no stdout on the watch) to diagnose whether it runs 4-wide or memory/`-101` steps it to 2. New pure `InstallPlan.effectiveMaxInFlight(memMax, ceiling)=min` (R1). Gates: 374 tests (+1); watchdog/indexload/install-check green (sim logs `fm:691320 inflight:4`). Modal `KEY_MENU` behaviour hardware-confirmed (KeyEvent unfakeable in a `:test`, per the M9.7 precedent). Binary-only (corpus v18 unchanged). | 374 |
